...
const FormularioRespuesta = ({ isOpen, onClose, onGuardar, tipoSolicitud = 'Respuesta a Oposición' }) => {
  ...
  const validate = (customForm) => {
    ...
    else if (!/^[A-Za-zÁÉÍÓÚáéíóúÑñ0-9 .,&-]{10,500}$/.test(f.argumentosRespuesta)) e.argumentosRespuesta = 'Mínimo 10 caracteres, máximo 500';
    if (!f.documentosRespuesta) e.documentosRespuesta = 'Adjunta los documentos';
    return e;
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-800 bg-opacity-60 backdrop-blur-sm transition-all">
      <div className="bg-white rounded-xl border border-gray-200 shadow-xl w-full max-w-3xl p-8 overflow-y-auto max-h-[90vh]">
        ...
        <div className="flex items-center gap-4 bg-gray-50 px-6 py-4 rounded-t-xl mb-6">
          ...
          <div>
            <h2 className="text-xl font-semibold text-gray-800">Solicitud de Respuesta a Oposición</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 rounded-lg p-4">
            ...
            <div>
              <label className="block text-sm font-medium mb-1">Marca que se opuso *</label>
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Argumentos de la Respuesta *</label>
              <textarea
              name="argumentosRespuesta" 
              value={form.argumentosRespuesta} 
              onChange={handleChange} 
              rows="4"
              placeholder="Describa los argumentos de su respuesta a la oposición..."
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium mb-1">Documentos de Respuesta *</label>
              <input type="file" name="documentosRespuesta" onChange={handleChange} className="w-full" />
              {errors.documentosRespuesta && <p className="text-xs text-red-600">{errors.documentosRespuesta}</p>}
            </div>
            {esRepresentante && (
              <>
                <div>
