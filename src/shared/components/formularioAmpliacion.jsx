...
const FormularioAmpliacion = ({ isOpen, onClose, onGuardar, tipoSolicitud = 'Ampliación de Marca' }) => {
  ...
  const validate = (customForm) => {
    ...
    if (!f.certificadoAmpliacion) e.certificadoAmpliacion = 'Adjunta el certificado';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire({
        ...
        text: 'La solicitud de ampliación se ha creado correctamente.'
      });
      onClose();
    } catch (err) {
      Swal.fire({
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-800 bg-opacity-60 backdrop-blur-sm transition-all">
      <div className="bg-white rounded-xl border border-gray-200 shadow-xl w-full max-w-3xl p-8 overflow-y-auto max-h-[90vh]">
        ...
        <div className="flex items-center gap-4 bg-gray-50 px-6 py-4 rounded-t-xl mb-6">
          ...
          <div>
            <h2 className="text-xl font-semibold text-gray-800">Solicitud de Ampliación de Marca</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium mb-1">Certificado de Ampliación *</label>
              <input type="file" name="certificadoAmpliacion" onChange={handleChange} className="w-full" />
              {errors.certificadoAmpliacion && <p className="text-xs text-red-600">{errors.certificadoAmpliacion}</p>}
            </div>
            <div>
