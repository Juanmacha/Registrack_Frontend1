...
const FormularioCesiondeMarca = ({ isOpen, onClose, onGuardar, tipoSolicitud = 'Cesión de Marca' }) => {
  ...
  const handleCesionarioChange = e => {
    ...
    setForm(f => {
      const updatedForm = { 
        ...f, 
        cesionario: { ...f.cesionario, [name]: value } 
      };
      const newErrors = validate(updatedForm);
      setErrors(newErrors);
      return updatedForm;
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-800 bg-opacity-60 backdrop-blur-sm transition-all">
      <div className="bg-white rounded-xl border border-gray-200 shadow-xl w-full max-w-3xl p-8 overflow-y-auto max-h-[90vh]">
        ...
        <div className="flex items-center gap-4 bg-gray-50 px-6 py-4 rounded-t-xl mb-6">
          ...
          <div>
            <h2 className="text-xl font-semibold text-gray-800">Solicitud de Cesión de Marca</h2>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-blue-50 rounded-lg p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Tipo de Documento *</label>
                <select name="tipoDocumento" value={form.cesionario.tipoDocumento} onChange={handleCesionarioChange} className={`w-full border rounded p-2 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 ${errors.cesionario_tipoDocumento ? 'border-red-500' : ''}`}>
                  <option value="">Seleccionar</option>
                  {tiposDocumento.map(t => <option key={t}>{t}</option>)}
                </select>
                {errors.cesionario_tipoDocumento && <p className="text-xs text-red-600">{errors.cesionario_tipoDocumento}</p>}
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Número de Documento *</label>
                <input type="text" name="numeroDocumento" value={form.cesionario.numeroDocumento} onChange={handleCesionarioChange} className={`w-full border rounded p-2 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 ${errors.cesionario_numeroDocumento ? 'border-red-500' : ''}`} />
                {errors.cesionario_numeroDocumento && <p className="text-xs text-red-600">{errors.cesionario_numeroDocumento}</p>}
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Correo Electrónico *</label>
                <input type="email" name="email" value={form.cesionario.email} onChange={handleCesionarioChange} className={`w-full border rounded p-2 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 ${errors.cesionario_email ? 'border-red-500' : ''}`} />
                {errors.cesionario_email && <p className="text-xs text-red-600">{errors.cesionario_email}</p>}
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Teléfono *</label>
                <input type="text" name="telefono" value={form.cesionario.telefono} onChange={handleCesionarioChange} className={`w-full border rounded p-2 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 ${errors.cesionario_telefono ? 'border-red-500' : ''}`} />
                {errors.cesionario_telefono && <p className="text-xs text-red-600">{errors.cesionario_telefono}</p>}
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Dirección *</label>
                <input type="text" name="direccion" value={form.cesionario.direccion} onChange={handleCesionarioChange} className={`w-full border rounded p-2 focus:ring-2 focus:ring-blue-400 focus:border-blue-400 ${errors.cesionario_direccion ? 'border-red-500' : ''}`} />
                {errors.cesionario_direccion && <p className="text-xs text-red-600">{errors.cesionario_direccion}</p>}
              </div>
            </div>
          </div>
          {/* Adjuntar Documentos */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium mb-1">Documento de Cesión *</label>
              <input type="file" name="documentoCesion" onChange={handleChange} className="w-full" />
              {errors.documentoCesion && <p className="text-xs text-red-600">{errors.documentoCesion}</p>}
            </div>
            {esRepresentante && (
              <>
                <div>
